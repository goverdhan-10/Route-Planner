...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function resetView() {
    ...
    document.querySelectorAll('.edge').forEach(edge => {
      edge.style.stroke = '#555';
        ...
        edge.classList.remove('highlighted-path');
    });
    
    // Reset permanent routes
    document.querySelectorAll('.permanent-route').forEach(route => {
  ...
  function showPath(path) {
    ...
    // Build a set of path edges (string keys "u-v")
    ...
    // Fade all edges to black with opacity 0.3
    document.querySelectorAll('.edge').forEach(edge => {
    ...
    // Fade permanent routes
    document.querySelectorAll('.permanent-route').forEach(route => {
        route.style.opacity = '0.2';
    });

    
    // Clear previous route and breakdown
    while(routeLayer.firstChild) routeLayer.removeChild(routeLayer.firstChild);
    pathBreakdown.innerHTML = '';
    
    if(!path.length) return;

    // Draw the path line (thicker and solid) - using gray color for the connecting line
    const points = path.map(i => `${stationCoords[i].x},${stationCoords[i].y}`).join(' ');
    const poly = document.createElementNS("http://www.w3.org/2000/svg","polyline");
    poly.setAttribute("points", points);
    poly.setAttribute("class", "route");
    poly.setAttribute("stroke", "#cccccc"); // Gray color for connecting segments
    ...
    path.forEach(stationId => {
        ...
        // Highlight station dots - using gray color
        ...
        dot.setAttribute("fill", "#cccccc"); // Gray color for dots
        routeLayer.appendChild(dot);
    });

    // Highlight the edges that are part of the path with their original colors
    document.querySelectorAll('.edge').forEach(edge => {
        ...
        // Match stations by coordinates
        const u = Object.keys(stationCoords).find(k => 
            approxEqual(stationCoords[k].x, x1) && approxEqual(stationCoords[k].y, y1));
        const v = Object.keys(stationCoords).find(k => 
            approxEqual(stationCoords[k].x, x2) && approxEqual(stationCoords[k].y, y2));
        
        if (u && v) {
            const key = u < v ? `${u}-${v}` : `${v}-${u}`;
            if (pathEdges.has(key)) {
                const route = permanentRoutes.find(r => 
                    r.path.some((s, i, arr) => 
                        (s == u && arr[i+1] == v) || (s == v && arr[i+1] == u)
                    ));
                
                edge.classList.add('highlighted-path');
                edge.style.stroke = route.color;
                edge.style.opacity = route ? '1':'0.5';
                edge.style.strokeWidth = route ? '8' : '1';
            }
        }
    });

    // Group the path by line segments
    const segments = [];
    let currentSegment = {
        lineIndex: -1,
        stations: [],
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        const lineIndex = permanentRoutes.findIndex(route => {
            const routePath = route.path;
            for (let j = 0; j < routePath.length - 1; j++) {
                if ((routePath[j] === stationId && routePath[j + 1] === nextStationId) ||
                    (routePath[j] === nextStationId && routePath[j + 1] === stationId)) {
                    return true;
                }
            }
            return false;
        ...
        if (lineIndex !== currentSegment.lineIndex) {
            ...
            currentSegment = {
                lineIndex,
                stations: [stationId],
                color: lineIndex >= 0 ? permanentRoutes[lineIndex].color : '#cccccc',
                name: lineIndex >= 0 ? permanentRoutes[lineIndex].name : 'Connecting Path'
            };
        } else {
            currentSegment.stations.push(stationId);
        }
    ...
    // Create the breakdown UI
    segments.forEach((segment, index) => {
        const segmentDiv = document.createElement('div');
        segmentDiv.className = 'line-segment';
        
        // Create color strip
        const colorStrip = document.createElement('div');
        colorStrip.className = 'line-color-strip';
        colorStrip.style.backgroundColor = segment.color;
        segmentDiv.appendChild(colorStrip);
        
        // Create line name and station count
        const infoDiv = document.createElement('div');
        infoDiv.className = 'line-info';
        
        const lineName = document.createElement('span');
        lineName.className = 'line-name';
        lineName.textContent = segment.name;
        infoDiv.appendChild(lineName);
        
        const countSpan = document.createElement('span');
        countSpan.className = 'stations-count';
        countSpan.textContent = ` (${segment.stations.length} ${segment.stations.length === 1 ? 'station' : 'stations'})`;
        infoDiv.appendChild(countSpan);
        
        segmentDiv.appendChild(infoDiv);
        
        // Create stations list (hidden by default)
        const stationsList = document.createElement('div');
        stationsList.className = 'stations-list';
        stationsList.style.display = 'none';
        
        segment.stations.forEach(stationId => {
            const stationItem = document.createElement('div');
            stationItem.className = 'station-item';
            stationItem.textContent = locationNames[stationId];
            stationsList.appendChild(stationItem);
        });
        
        // Toggle stations list on click
        countSpan.addEventListener('click', () => {
            stationsList.style.display = stationsList.style.display === 'none' ? 'block' : 'none';
        });
        
        segmentDiv.appendChild(stationsList);
        pathBreakdown.appendChild(segmentDiv);
        
        // Add "Change here" text between segments
        if (index < segments.length - 1) {
            const changeDiv = document.createElement('div');
            changeDiv.className = 'change-here';
            ...
            changeText.textContent = 'Change here';
            changeDiv.appendChild(changeText);
            
            pathBreakdown.appendChild(changeDiv);
        }
    });
...
>>>>>>> edcfdacf33b27526ea3301c77a5dddbcb524767e
